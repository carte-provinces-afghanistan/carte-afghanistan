...
function clampTransform(transform: MapTransform): MapTransform {
  ...
  return {
    ...
    y: clamp(transform.y, MAP_HEIGHT - MAP_HEIGHT * scale, 0)
  };
}
...
export function MapView({ selectedCode, onSelectProvince }: MapViewProps) {
  ...
  const neighbourCountryFeatures = useMemo(() => {
    return (neighbourCountryCollection?.features ?? []).map((feature) => ({
      feature,
      labelPoint: toScreenPoint(
        neighbourLabelAnchors[feature.properties.iso3],
        projection
      )
    }));
  ...
  function handlePointerMove(event: PointerEvent<SVGSVGElement>) {
    ...
    const svgDeltaX = ((nextCentroid[0] - gesture.lastCentroid[0]) * MAP_WIDTH) / bounds.width;
    const svgDeltaY = ((nextCentroid[1] - gesture.lastCentroid[1]) * MAP_HEIGHT) / bounds.height;
